...
const AddProductScreen = ({ navigation }: Props) => {
  ...
  useEffect(() => {
    if (pendingProduct?.preco != null) {
      setValorUnitario(
        pendingProduct.preco.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
      );
    }
  }, [pendingProduct]);

  useEffect(() => { return () => { setPendingProduct(null); }; }, []);

  const toggleVendedor = (id: number) =>
    setVendedoresSelecionados(prev => prev.includes(id) ? prev.filter(v => v !== id) : [...prev, id]);

  const handleSave = () => {
    if (!pendingProduct) { Alert.alert('Atenção', 'Por favor, selecione um produto'); return; }
  ...
  const footerProps = {
    onPress: handleSave, loading: false, text: 'Adicionar',
    quantidade: parseQuantity(quantidade),
    valorUnitario: parseCurrency(valorUnitario),
    desconto: desconto ? parseQuantity(desconto) : 0,
  };
          ...
          >
            <View style={styles.formContainer}>
              <FormField label="Produto" required>
                <TouchableOpacity style={styles.productSelector} onPress={() => navigation.navigate('ProductSearch')}>
                  {pendingProduct ? (
...
const makeStyles = (colors: AppColors) =>
  StyleSheet.create({
    ...
    productSelectorText: { fontSize: 16, color: colors.textPrimary, flex: 1, marginRight: 8 },
    productSelectorPlaceholder: { fontSize: 16, color: colors.textPlaceholder, flex: 1 },
  });
